...
export default function AdminLoginPage() {
  ...
  return <section className="panel compact login-panel"><p className="eyebrow">ADMIN AUTH</p><h1>管理员登录</h1><p className="muted">使用服务器验证的管理员密码进入签到工作台。</p><label>管理员密码<input type="password" value={password} onChange={(event) => setPassword(event.target.value)} autoComplete="current-password" /></label>{error ? <p className="muted">{error}</p> : null}<button className="button" disabled={busy || !password} onClick={() => void submit()}>{busy ? '验证中…' : '登录'}</button></section>
}
