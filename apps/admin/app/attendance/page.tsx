...
export default function AttendancePage() {
  ...
  useEffect(() => {
    void fetch(`${apiBase}/api/v1/projects`, { credentials: 'include' }).then((response) => response.json() as Promise<{ items: Project[] }>).then(async (value) => {
      setProjects(value.items)
      const values = (await Promise.all(value.items.map((project) => fetch(`${apiBase}/api/v1/projects/${project.id}/sessions`, { credentials: 'include' }).then((response) => response.json() as Promise<{ items: Session[] }>)))).flatMap((item) => item.items).sort((a, b) => a.scheduledStartAt.localeCompare(b.scheduledStartAt))
      setSessions(values); setSelected(values[0]?.id ?? '')
    }).catch(() => setError('API 暂时不可用'))
  }, [])
  const refresh = async (id = selected) => { if (!id) return; const response = await fetch(`${apiBase}/api/v1/sessions/${id}/attendance/live`, { headers: adminHeaders, credentials: 'include' }); if (!response.ok) throw new Error('live failed'); setLive(await response.json() as Live) }
  useEffect(() => { if (!selected || !live) return; const timer = window.setInterval(() => { void fetch(`${apiBase}/api/v1/sessions/${selected}/attendance/live`, { headers: adminHeaders, credentials: 'include' }).then((response) => response.json() as Promise<Live>).then(setLive).catch(() => undefined) }, 5000); return () => window.clearInterval(timer) }, [selected, live])
  const start = async () => { if (!selected) return; const response = await fetch(`${apiBase}/api/v1/sessions/${selected}/attendance/start`, { method: 'POST', headers: adminHeaders, credentials: 'include', body: JSON.stringify({ durationMinutes: 10 }) }); if (!response.ok) throw new Error('start failed'); await refresh() }
  const setStatus = async (projectMemberId: string, status: Record['status'] | 'VOID') => { const response = await fetch(`${apiBase}/api/v1/sessions/${selected}/attendance`, { method: 'PATCH', headers: adminHeaders, credentials: 'include', body: JSON.stringify({ projectMemberId, status }) }); if (!response.ok) throw new Error('update failed'); await refresh() }
  const finalize = async () => { const response = await fetch(`${apiBase}/api/v1/sessions/${selected}/attendance/finalize`, { method: 'POST', headers: adminHeaders, credentials: 'include', body: '{}' }); if (!response.ok) throw new Error('finalize failed'); setLive(await response.json() as Live) }
  const current = sessions.find((session) => session.id === selected)
  return <>
    <header className="page-header"><div><p className="eyebrow">ATTENDANCE WORKBENCH</p><h1>签到工作台</h1><p className="muted">从已发布课表直接发起 NORMAL 签到。</p></div><Link href="/login">管理员登录</Link></header>
